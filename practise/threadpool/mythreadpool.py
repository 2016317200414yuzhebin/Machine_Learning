...
class MyThreadPool(object):
    ...
    # 获取下载任务
    def get_task(self):
        # 从列表中取元素，如果 block = True 则一直阻塞到有可用元素为止
    ...
    def wait_all_complete(self):
        for item in self.threads:
            if item.isAlive():
                # join函数的意义，只有当前执行join函数的线程结束程序才能接着执行下去
                item.join()
...
class WorkThread(threading.Thread):
    ...
    # 定义线程功能方法，当thread_l,...,thread_n,调用start()之后执行操作
    def run(self):
        print(threading.current_thread().getName())
        while True:
            try:
                # get_task()从工作队列里获取当前正在下载的线程,格式为func,url,deep
